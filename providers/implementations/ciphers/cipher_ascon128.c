                                      ...
                                      const OSSL_PARAM params[])
{
    ...
    if (nonce != NULL)
    {
        ...
        {
            ERR_raise(ERR_LIB_PROV, ASCON_NONCE_INCORRECT_LEN);
            return OSSL_RV_ERROR;
        }
    }
                        ...
                        size_t outsize, const unsigned char *in, size_t inl)
{
    ...
    {
        ERR_raise(ERR_LIB_PROV, ASCON_NO_ONGOING_OPERATION);
        return OSSL_RV_ERROR;
    }
...
int ossl_cipher_ascon128_final(void *vctx, unsigned char *out, size_t *outl, size_t outsize)
{
    ...
    else if (ctx->direction == DECRYPTION)
    {
        ...
        {
            ERR_raise(ERR_LIB_PROV, ASCON_NO_TAG_SET);
            return OSSL_RV_ERROR;
        }
    }
    ...
    for (p = params; p->key != NULL; p++) {
        if (strcmp(p->key, OSSL_CIPHER_PARAM_AEAD_AAD) == 0) {
            ...
            if (ctx->assoc_data_processed)
            {
                ...
                ERR_raise(ERR_LIB_PROV, ASCON_ONGOING_OPERATION);
                ok = 0;
                break;
            }
            ...
            {
                ERR_raise(ERR_LIB_PROV, ASCON_ONLY_FIXED_TAG_LENGTH_SUPPORTED);
                ok = 0;
                break;
            }
            ctx->tag_len = tag_len;
